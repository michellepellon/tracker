...
- **Preflight + Doctor now catch unborn HEAD (no commits) up front** (PR #235 round 7, Copilot:3260568737). `git rev-parse --is-inside-work-tree` returns true for a `git init`'d repo that has no commits, so pre-fix a `requires: git` workflow could pass preflight against an unborn-HEAD repo and crash mid-run on `git worktree add ... HEAD`, `git merge`, or `git log` after burning LLM turns. The new `pipeline.HasBornHEAD` probe (`git rev-parse --verify HEAD`) runs after `--is-inside-work-tree` succeeds; on unborn HEAD Preflight returns `ErrGitUnbornHEAD` and Doctor reports Error with copy-paste remediation (`git commit --allow-empty -m initial` for an empty baseline, or `git add . && git commit -m initial` to capture existing files). The manual not-a-repo remediation in `buildWorkdirNotRepoMessage` was updated to chain `git init` with `git commit --allow-empty -m initial` so users following the docs land in the same born-HEAD state that `--git=init --allow-init` produces. `--git=warn` downgrades both to a warning as before.
