...
- **Engine wires spec loading + status reporter** (PR3 of the spec-first arc). When a workflow declares `spec: <loader> <path>`, `LoadDippinWorkflowFromIR` resolves the loader, parses the spec, and stashes it on `Graph.Spec`. Every node's `satisfies:` is validated at load time — bare unknown ACIDs are fatal; wildcard / range patterns that resolve to nothing become `TRK-SAT` warnings. At `Engine.Run` start, the matching reporter's `Pull` seeds `PipelineContext` internal keys (`spec.status.<acid>`) from server-side state. After each successful node with non-empty `Satisfies`, the engine batches the resolved ACIDs into one `Push` call (StatePass, comment `node:<id>`). All reporter calls are best-effort — missing token, missing binary, transport errors emit a warning event and continue. The reporter `Target.Implementation` defaults to the current git branch (fallback `"unknown"`).
- **Agent prompt injection on `satisfies:`** (PR4 of the spec-first arc). Before each node's handler runs, the engine populates two PipelineContext keys with the requirement slice resolved from the node's `satisfies:` patterns: `spec.requirements` (deterministic YAML, sorted by ACID) and `spec.requirements_json` (same data as JSON). Workflow authors interpolate via `${ctx.spec.requirements}` in their prompt to receive structured ACID context — no manual wiring. Both keys are node-scoped: nodes without `satisfies:` get them cleared so prior nodes' content cannot bleed in. Wildcards and ranges resolve to all matching requirements; the engine does not filter deprecated requirements (authors can post-process if desired).
- **Per-ACID status readable from edge conditions** (PR5 of the spec-first arc). PR3 already populated `spec.status.<acid>` context keys from the reporter's `Pull` at workflow start; this PR promotes them from the internal context store to the user-visible store (via `MergeWithoutDirty` so the keys stay global instead of bleeding into per-node scoped namespaces). Workflow authors can now route edges with `when ctx.spec.status.foo.BAR.1 = pass` — no new evaluator syntax, the existing operator vocabulary handles it. Values are the lowercase `String()` form of `reporter.State` (`pass`, `fail`, `blocked`, `pending`, `unknown`). Absent ACIDs return empty string (lenient evaluator behaviour), so `... != pass` is a safe "not yet done" route.
- **`verify_acid:` tool-node primitive** (PR5.5 of the spec-first arc). Consumes the dippin grammar shipped in [michellepellon/dippin-lang@4b620b7](https://github.com/michellepellon/dippin-lang/commit/4b620b7). After a successful tool node with `verify_acid:`, the engine resolves the listed patterns against `Graph.Spec`, walks the working tree, and writes per-ACID coverage results into `spec.coverage.<acid>` (values: `covered` or `uncovered`). Outcome is **NOT** modified — exit code remains the source of truth for node success. Authors route via `when ctx.spec.coverage.foo.BAR.1 = uncovered`. The walker skips common noise dirs (`.git`, `node_modules`, `vendor`, etc.), binary extensions (`.png`, `.zip`, etc.), and files over 1 MiB. Load-time validation: bare unknown ACIDs in `verify_acid:` are fatal (matches `satisfies:`); empty wildcards/ranges become `TRK-VAC` warnings. The replace directive in `go.mod` is bumped to the new dippin SHA.
- **`ship_acai_spec` built-in workflow** (PR6, final of the spec-first arc). New template workflow demonstrating the canonical spec-first authoring pattern end-to-end: an `Implement` agent reads requirements via `${ctx.spec.requirements}`, a `Verify` tool node greps for ACID coverage, conditional edges route to a `FixGaps` agent (via `restart:` back-edge) when coverage is incomplete, and the engine's auto-push reports status to the acai dashboard after each success. Ships as `examples/ship_acai_spec.dip` + `workflows/ship_acai_spec.dip` (embedded in the binary, runnable via `tracker ship_acai_spec`). Includes a load test (`pipeline/spec_ship_test.go`) that exercises the full composition against a real cognitoforms-py `features.yaml`. Author-facing guide at `docs/spec-first-authoring.md`.
